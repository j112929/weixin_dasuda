...
public class Constants {
    private final static Log log = LogFactory.getLog(Constants.class);
...
//    private List<Fan> fans;
    ...
    public Constants(String user, String pwd) {
        Constants.loginUser = user;
        Constants.loginPwd = pwd;
    }
    ...
    public void setLoginErrCode(int loginErrCode) {
        Constants.loginErrCode = loginErrCode;
    }
    ...
    public void setLoginErrMsg(String loginErrMsg) {
        Constants.loginErrMsg = loginErrMsg;
    }
    ...
    public void setMsgSendCode(int msgSendCode) {
        Constants.msgSendCode = msgSendCode;
    }
    ...
    public void setMsgSendMsg(String msgSendMsg) {
        Constants.msgSendMsg = msgSendMsg;
    }
    ...
    public void login() {
        boolean bool = _login();
        while (!bool) {
            String info = "【登录失败】【错误代码：" + Constants.loginErrMsg + "】账号【" + Constants.loginUser + "】正在尝试重新登�?...";
            log.debug(info);
...
//        System.out.println("登陆成功");
    ...
    public static boolean _login() {
        try {
            ...
            NameValuePair[] params = new NameValuePair[] { new NameValuePair("username", Constants.loginUser),
                    new NameValuePair("pwd", DigestUtils.md5Hex(Constants.loginPwd.getBytes())), new NameValuePair("f", "json"),
...
//                 System.out.println(retcode.getRet());
                ...
                if ((retcode.getBase_resp().getRet() == 302 || retcode.getBase_resp().getRet() == 0)) {
                	Constants.cookies = client.getState().getCookies();
                    StringBuffer cookie = new StringBuffer();
                    for (Cookie c : client.getState().getCookies()) {
                        cookie.append(c.getName()).append("=").append(c.getValue()).append(";");
                        cookiemap.put(c.getName(), c.getValue());
                    }
                    Constants.cookiestr = cookie.toString();
                    Constants.isLogin = true;
                    Constants.token = getToken(retcode.getRedirect_url());
                    return true;
                }
                int errCode = 0;
                Constants.loginErrCode = errCode;
                switch (errCode) {

                case -1:
                    Constants.loginErrMsg = "系统错误";
                    return false;
                case -2:
                    Constants.loginErrMsg = "帐号或密码错";
                    return false;
                case -23:
                    Constants.loginErrMsg = "密码错误";
                    return false;
                case -21:
                    Constants.loginErrMsg = "不存在该帐户";
                    return false;
                case -5:
                    Constants.loginErrMsg = "访问受限";
                    return false;
                case -6:
                    Constants.loginErrMsg = "请输入验证";
                    return false;
                case -7:
                    Constants.loginErrMsg = "此帐号已绑定私人微信号，不可用于公众平台登录";
                    return false;
                case -8:
                    Constants.loginErrMsg = "邮箱已存";
                    return false;
                case -32:
                    Constants.loginErrMsg = "验证码输入错误";
                    return false;
                case -200:
                    Constants.loginErrMsg = "因频繁提交虚假资料，该帐号被拒绝登录";
                    return false;
                case -94:
                    Constants.loginErrMsg = "请使用邮箱登录";
                    return false;
                case 10:
                    Constants.loginErrMsg = "该公众会议号已经过期，无法再登录使用";
                    return false;
                case 65201:
                case 65202:
                    Constants.loginErrMsg = "成功登陆，正在跳�?..";
                    return true;
                case 0:
                    Constants.loginErrMsg = "成功登陆，正在跳�?..";
                    return true;
                default:
                    Constants.loginErrMsg = "未知的返回值";
                    return false;
                }
            }
        } catch (Exception e) {
...
//            System.err.println(info);
    ...
    public void msgpost() throws Exception { 
        String at = Constants.getAccessToken("wx84b61f062f6806ef", "cc9c53b812205bc6c7fbf6f369192482");
        if (at  != null) {                   
...
//        System.out.println(status);//200=SC_OK
  ...
  public static String getTicket() throws Exception{
    ...
    /**
     * 说明:<br>
     * new Constants()对象，先登录再取粉丝数和者发消息。<br>
...
//        String LOGIN_PWD = "AAbb1122";
        ...
        Constants wx = new Constants(LOGIN_USER, LOGIN_PWD);
//        wx.login();
//        wx.getCookiestr();        
        wx.msgpost();
//        wx.msgpostbygroup();
